...
<div class="item_contacts" <?=$WITH_FORM?>>
    ...
    <form id="<?=$FORM_ID?>"
        ...
        <?if(!empty($arResult['FORM_FIELDS'])):?>
                ...
                <?foreach($arResult['FORM_FIELDS'] as $fieldCode => $arField):?>
                    ...
                    <?if($arField['TYPE'] == 'textarea'):?>
                        <div class="col-xs-12">
                            <div class="form-group">
                                ...
                                <? endif; ?>
                                <textarea class="textarea_site" id="<?=$arField['ID']?>" rows="5" name="<?=$arField['NAME']?>" <?=$arField['PLACEHOLDER_STR'];?> <?=$arField['REQ_STR']?>><?=$arField['VALUE'];?></textarea>
                            </div>
                        </div>
                    ...
                    <?elseif($arField['TYPE'] == 'accept'):?>
                        <div class="col-xs-12">
                            <label class="label_wrapper">
                                <input  type="checkbox" class='checkbox' name="<?=$arField['NAME']?>" value="<?=Loc::getMessage('SLAM_EASYFORM_YES')?>" <?=$arField['REQ_STR']?> checked>
                                <span class='checkbox-custom'></span>
                                <span class='label_text'><?=htmlspecialcharsBack($arField['VALUE'])?></span>
                            </label>
                        </div>
                    ...
                    <?else:?>
                        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                            <input class="input_site<?=($arField['TYPE'] == 'text')?" name":""?>" type="<?=$arField['TYPE'];?>" id="<?=$arField['ID']?>" name="<?=$arField['NAME']?>" value="<?=$arField['VALUE'];?>" <?=$arField['PLACEHOLDER_STR'];?> <?=$arField['REQ_STR']?> <?=$arField['MASK_STR']?>>
                        </div>
                    <?endif;?>
                <?endforeach;?>
            </div>
            <?/*foreach($arResult['FORM_FIELDS'] as $fieldCode => $arField):
                ...
                <?elseif($arField['TYPE'] == 'accept'):?>
                    <label class="label_wrapper">
                        <input  type="checkbox" class='checkbox' name="<?=$arField['NAME']?>" value="<?=Loc::getMessage('SLAM_EASYFORM_YES')?>" <?=$arField['REQ_STR']?>>
                        <span class='checkbox-custom'></span>
                        <span class='label_text'><?=htmlspecialcharsBack($arField['VALUE'])?></span>
                    </label>
                    <br>
            ...
            <button type="submit" class="btn_site btn_red" data-default="<?=$arParams['FORM_SUBMIT_VALUE']?>">
