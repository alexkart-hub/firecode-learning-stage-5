...
if (!empty($arResult['ITEMS']))
{
	...
	$firstNum = rand(1,$countItems-1);
    $countItems-$secondNum = ($firstNum + 1 <= $countItems-1) ? $firstNum + 1 : $firstNum - 1;
    $thirdNum = ($countItems-$secondNum <> $secondNum) ? $countItems-$secondNum : $countItems-$firstNum;
	$currentNum = 1;
    foreach ($arResult['ITEMS'] as $key=>$item){
        if($item['ID']!=$arParams['CURRENT_ELEMENT_ID']){
            if(in_array($currentNum,array($firstNum,$secondNum,$thirdNum))){
